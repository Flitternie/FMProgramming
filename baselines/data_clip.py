...
client = MilvusClient("image_retrieval_clip.db")
...
for image_id in folders:
    ...
    for img in range(len(positive_tensors[0])):
        img_embedding = get_single_image_embedding(positive_tensors[0][img], clip_processor, clip_model, device)
        client.insert(
                f"img_{image_id}",
                {
    ...
    for img in range(len(negative_tensors[0])):
        try:
            img_embedding = get_single_image_embedding(negative_tensors[0][img], clip_processor, clip_model, device)
            client.insert(
                    f"img_{image_id}",
                    {
                        "vector": img_embedding.flatten().tolist(), 
