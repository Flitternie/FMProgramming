...
class ModifiedSupervisedContextual: # !!!
    def __init__(self, num_arms, input_size, hidden_size=512, device=device):
        ...
        # Arm transformation
        self.arm_transform = nn.Linear(self.hidden_size, self.num_arms).to(self.device)
    ...
    def train(self, contexts, arm_indices, rewards, num_epochs=5, batch_size=16, lr=0.0003):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_contexts, batch_arms, batch_rewards in pbar:
                ...
                arm_idx_hot = torch.zeros(len(batch_arms), self.num_arms, device=self.device)
                arm_idx_hot.scatter_(1, batch_arms.unsqueeze(1), 1)
                predicted_rewards = torch.sum(self.arm_transform(transformed_contexts) * arm_idx_hot, dim=1)
    ...
    def select(self, context):
        ...
        arm_idx_hot = torch.eye(self.num_arms, device=self.device)
        predicted_rewards = torch.sum(self.arm_transform(transformed_context.repeat(self.num_arms, 1)) * arm_idx_hot, dim=1)
