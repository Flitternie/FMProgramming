...
class LegacyCNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.fc1 = nn.Linear(128 * 32 * 32, 256)  # Adjusted for feature map size after pooling
        self.fc2 = nn.Linear(256, num_classes)

    def forward(self, x):
        # Input shape: [batch_size, 3, 256, 256]
        x = self.pool(F.relu(self.conv1(x)))  # Output: [batch_size, 32, 128, 128]
        x = self.pool(F.relu(self.conv2(x)))  # Output: [batch_size, 64, 64, 64]
        x = self.pool(F.relu(self.conv3(x)))  # Output: [batch_size, 128, 32, 32]
        
        # Flatten feature maps while preserving batch size
