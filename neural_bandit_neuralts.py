...
class NeuralTS(RandomAlg):
    ...
    def take_action(self, context):
        ...
        with torch.no_grad():
            ...
            loc = self.net(context).squeeze()  # Shape: [K]

            # Compute the scale (standard deviation) for the normal distribution
            gs = torch.matmul(g, self.sigma_inv)  # Shape: [K, numel]
            scale_squared = (gs * g).sum(dim=1)  # Shape: [K]
            scale = self.beta * torch.sqrt(scale_squared)
    ...
    def update(self, context, action, reward):
        ...
        self.replay_buffer.add(context[action].cpu().to(torch.float32).numpy(), reward)
